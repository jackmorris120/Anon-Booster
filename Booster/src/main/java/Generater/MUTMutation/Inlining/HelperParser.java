...
public class HelperParser {
    ...
    /**
     * Extracts test file paths from bug triggering tests string using
     * ASTParser.convertClassToPath
     ...
     * @param bugTriggeringTestFilePaths Set of file paths that contain bug
     *                                   triggering tests
     * @param limitToBugTriggeringTests  Whether to limit scope to bug triggering
     *                                   test files
     * @return true if tests from this file should be inlined
     */
    private static boolean isTestFileInBugTriggeringList(String fileName, Set<String> bugTriggeringTestFilePaths,
            boolean limitToBugTriggeringTests) {
        ...
        if (bugTriggeringTestFilePaths.isEmpty()) {
            // If no bug triggering test files specified, inline all tests (backward
            // compatibility)
            return true;
        }
    ...
    public void run() {
        ...
        // Check if we should limit scope to bug triggering test files (file-level
        // filtering)
        ...
        if (limitToBugTriggeringTestFiles) {
            ...
            if (!bugTriggeringTestFilePaths.isEmpty()) {
                System.out.println("FILE-LEVEL FILTERING: Enabled - will only inline from bug triggering test files: "
                        + bugTriggeringTestFilePaths.size() + " files");
                for (String filePath : bugTriggeringTestFilePaths) {
                    System.out.println("  Target file: " + filePath);
                }
            } else {
                System.out.println(
                        "FILE-LEVEL FILTERING: Enabled but no bug triggering test files specified - will inline all test methods (backward compatibility)");
            }
        }
        ...
        for (Map.Entry<String, List<CtType<?>>> entry : fileTypeMap.entrySet()) {
            ...
            // Check if this entire file should be processed based on bug triggering test
            // list
            boolean processThisFile = isTestFileInBugTriggeringList(fileName, bugTriggeringTestFilePaths,
                    limitToBugTriggeringTestFiles);
                ...
                for (CtMethod<?> method : type.getMethods()) {
                    if (isTestMethod(method)) {
                        ...
                        if (processThisFile) {
                            ...
                            String testIdentifier = method.getDeclaringType().getQualifiedName() + "::"
                                    + method.getSimpleName();
                            String reason = limitToBugTriggeringTestFiles ? "(from bug triggering file)"
                                    : "(all tests)";
                            // System.out.println(" [DEBUG] Will inline test: " + testIdentifier + " " +
                            // reason);
                        }
                    }
                }
        ...
        if (limitToBugTriggeringTestFiles) {
            System.out.println("Found " + totalTestMethods + " total test methods, filtering to " + filteredTestMethods
                    + " test methods from bug triggering files");
        } else {
            System.out.println("Found " + totalTestMethods + " total test methods, processing all "
                    + filteredTestMethods + " test methods");
        }
        ...
        // System.out.println(" File: " + fileName + " (" + testCount + " test
        // methods)");
        // }
        // }
            ...
            for (Map.Entry<String, List<CtMethod<?>>> entry : testsByFile.entrySet()) {
                ...
                if (i < methodList.size()) {
                    if (System.currentTimeMillis() - inLineStartTime > 200000 || currentCount > 3000) {
                        System.out.println("Process stopped due to timeout or method limit : "
                                + (System.currentTimeMillis() - inLineStartTime) + " ms");
                        return;
                    }

                    CtMethod<?> method = methodList.get(i);

                    // Debug: Track testInnerEmptyNamespaceDOM
                    boolean isTargetTest = method.getSimpleName().contains("testInnerEmptyNamespaceDOM");
                    if (isTargetTest) {
                        System.out.println("\n[DEBUG_HELPER] ========================================");
                        System.out.println("[DEBUG_HELPER] Processing test: " + method.getSimpleName());
                        System.out.println(
                                "[DEBUG_HELPER] Declaring class: " + method.getDeclaringType().getQualifiedName());
                    }
                    ...
                    if (isTargetTest) {
                        System.out.println("[DEBUG_HELPER] Found " + setupChain.size() + " setup methods");
                    }
                    ...
                    if (isTargetTest) {
                        System.out.println("[DEBUG_HELPER] Removing try-catch and assertions...");
                    }
                    removeTryCatchAndAssertion(method);

                    if (isTargetTest) {
                        System.out.println("[DEBUG_HELPER] Starting recursive inlining...");
                    }
                    expandAndInlineRecursively(method, new HashSet<>());

                    if (isTargetTest) {
                        System.out.println("[DEBUG_HELPER] Ensuring all variables initialized...");
                    }
                    // 초기화되지 않은 변수를 모두 타입에 맞게 초기화
                    ensureAllVariablesInitialized(method, this.methodInliner.getFactory());
        ...
        for (CtFieldReference<?> fieldRef : fieldsToInline) {
            ...
            if (finalInitializer != null
                ...
                if (initializerType != null) {
                    CtTypeReference<?> qualifiedTypeRef = factory.Type()
                            .createReference(initializerType.getQualifiedName());
                ...
                if (!generatedStmts.isEmpty()) {
                    ...
                    for (Map.Entry<String, CtFieldReference<?>> entry : fieldsToLocalizeMap.entrySet()) {
                        ...
                        if (fieldTypeToLocalize != null) {
                            CtTypeReference<?> qualifiedType = factory.Type()
                                    .createReference(fieldTypeToLocalize.getQualifiedName());
                        ...
                        for (CtStatement stmt : generatedStmts) {
                            for (CtFieldAccess<?> fieldAccess : stmt
    ...
    private void renameSetupLocalVariables(CtBlock<?> body, Map<String, String> renameMap) {
        ...
        List<CtLocalVariable<?>> declarations = new ArrayList<>(
                body.getElements(new TypeFilter<>(CtLocalVariable.class)));
    ...
    /**
     * 초기화되지 않은 지역 변수를 찾아서 타입에 맞는 기본값으로 초기화합니다.
     * 원본 테스트에서 선언만 되고 초기화되지 않은 변수를 처리합니다.
     */
    private void ensureAllVariablesInitialized(CtMethod<?> method, Factory factory) {
        List<CtLocalVariable<?>> allVars = method.getElements(new TypeFilter<>(CtLocalVariable.class));

        for (CtLocalVariable<?> var : allVars) {
            // 초기화 표현식이 없는 변수 찾기
            if (var.getDefaultExpression() == null) {
                CtTypeReference<?> type = var.getType();
                if (type == null) {
                    continue; // 타입을 알 수 없으면 건너뜀
                }

                CtExpression<?> defaultValue;

                if (type.isPrimitive()) {
                    // Primitive 타입에 맞는 기본값
                    switch (type.getSimpleName()) {
                        case "boolean":
                            defaultValue = factory.createLiteral(false);
                            break;
                        case "byte":
                            defaultValue = factory.createLiteral((byte) 0);
                            break;
                        case "short":
                            defaultValue = factory.createLiteral((short) 0);
                            break;
                        case "int":
                            defaultValue = factory.createLiteral(0);
                            break;
                        case "long":
                            defaultValue = factory.createLiteral(0L);
                            break;
                        case "float":
                            defaultValue = factory.createLiteral(0.0f);
                            break;
                        case "double":
                            defaultValue = factory.createLiteral(0.0d);
                            break;
                        case "char":
                            defaultValue = factory.createLiteral('\u0000');
                            break;
                        default:
                            defaultValue = factory.createLiteral(null);
                    }
                } else {
                    // Reference 타입은 null
                    defaultValue = factory.createLiteral(null);
                ...
                var.setDefaultExpression((CtExpression) defaultValue);
            }
        }
    }
    ...
    private boolean isHelperCallToInline(CtInvocation<?> invocation, Set<String> callStack) {
        ...
        // Check if declaring class or any of its subclasses in the hierarchy is in
        // targetClassFqns
    ...
    /**
     * Check if the given class or any of its subclasses in the hierarchy is in
     * targetClassFqns.
     * This allows inlining helper methods from parent classes like BaseMapTest,
     * BaseTest, etc.
     */
    private boolean isInTargetClassHierarchy(CtType<?> declaringClass) {
        ...
        // This handles the case where BaseMapTest is extended by
        // TestTreeMapperDeserializer
    ...
    /**
     * Check if subclass is a subtype of superclass (including transitive
     * inheritance).
     */
